...
typedef enum InstType { R =82, I=73, S=83, B=66, U=85, J=74} InstType;
...
//    - int: end [last position of the number to be collected]
//
// Returns: (answer)
unsigned pack(int number, int answer, int begin, int end){
    unsigned int temp=0;
...
int main()
{
    ...
    char types[7] = "RISBUJ";
    if (strcmp_custom(&types[0], &data.type, 1) == 0){ //R
        answer = pack(data.funct7, answer, 25, 31); 
        answer = pack(data.rs2, answer, 27, 31); 
        answer = pack(data.rs1, answer, 27, 31); 
        answer = pack(data.funct3, answer, 29, 31);
        answer = pack(data.rd, answer, 27, 31);
        answer = pack(data.opcode, answer, 25,31);

    } else if (strcmp_custom(&types[1], &data.type, 1) == 0){ //I
        answer = pack(data.imm, answer, 20, 31);
        answer = pack(data.rs1, answer, 27, 31);
        answer = pack(data.funct3, answer, 29, 31);
        answer = pack(data.rd, answer, 27, 31);
        answer = pack(data.opcode, answer, 25,31);

    } else if (strcmp_custom(&types[2], &data.type, 1) == 0){ //S
        answer = pack(data.imm, answer, 20, 26); 
        answer = pack(data.rs2, answer, 27, 31); 
        answer = pack(data.rs1, answer, 27, 31); 
        answer = pack(data.funct3, answer, 29, 31);
        answer = pack(data.imm, answer, 27, 31);
        answer = pack(data.opcode, answer, 25,31);

    }else if (strcmp_custom(&types[3], &data.type, 1) == 0){ //B
        answer = pack(data.imm, answer, 19, 19); 
        answer = pack(data.imm, answer, 21, 26); 
        answer = pack(data.rs2, answer, 27, 31); 
        answer = pack(data.rs1, answer, 27, 31); 
        answer = pack(data.funct3, answer, 29, 31);
        answer = pack(data.imm, answer, 27, 30);
        answer = pack(data.imm, answer, 20, 20); 
        answer = pack(data.opcode, answer, 25,31);


    }else if (strcmp_custom(&types[4], &data.type, 1) == 0){ //U
        answer = pack(data.imm, answer, 12, 31); 
        answer = pack(data.rd, answer, 27, 31);
        answer = pack(data.opcode, answer, 25,31);

    }else if (strcmp_custom(&types[5], &data.type, 1) == 0){ //J
        answer = pack(data.imm, answer, 11, 11); 
        answer = pack(data.imm, answer, 21, 30); 
        answer = pack(data.imm, answer, 20, 20); 
        answer = pack(data.imm, answer, 12, 19); 
        answer = pack(data.rd, answer, 27, 31);
        answer = pack(data.opcode, answer, 25,31);
    }
    hex_code(answer);
